...
function Dashboard({ Logout, userdata }) {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
// role
const [role,setrole]=useState({email:userdata.email,role:localStorage.getItem("role") || "ADMIN"})
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-slate-800 flex overflow-hidden">
      ...
      >
        ...
        <div className="h-20 flex items-center px-6 border-b border-slate-800">
          ...
          <div
            className={`font-bold text-lg tracking-wide whitespace-nowrap overflow-hidden transition-all duration-300 ${
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-2">
          <NavItem
            ...
            label="Summary"
            active
            fullWidth={isSidebarOpen || isMobile}
          />
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <StatCard
                ...
                trend="-2%"
                trendUp={false}
                icon={<CircleCheckBig className="text-blue-500" />}
                color="bg-blue-50"
              />
              <StatCard
                title="PROJECT UNCOMPLETED"
                value="95"
                trend="-2%"
                trendUp={false}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <div className="xl:col-span-2 space-y-6">
                ...
                {role.role =="ADMIN" &&  <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-4">
                    {userdata.usersdataset?.map((item, index) => (
                      
                      <TransactionItem
                        key={index}
                        title={item.name}
                        isPositive={true}
                        time={`# Account creation: ${item.createdAt.split("T")[0].split("-").reverse().join("/")} # Time: ${item.createdAt.split("T")[1].split(".")[0]}`}
                        // amount="Edit"
                        icon={<User className="rotate-0" size={20} />}
                      />
                    ))}

                    {/* <TransactionItem
                      title="Dinesh"
                      time="Account creartion date: 12:40 am"
                      amount="Assign task: 5"
                      isPositive={true}
                      icon={<User className="rotate-0" size={20} />}
                    /> */}
                    {/* <TransactionItem
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-indigo-600 to-blue-600 rounded-2xl p-6 text-white shadow-lg shadow-blue-200 relative overflow-hidden">
                  ...
                  <p className="text-blue-100 text-sm mb-6 relative z-10">
                    Get 100% insurance for your goods and unlimited analytics.
                  </p>
...
const NavItem = ({
  ...
  >
    ...
    <span
      className={`ml-3 font-medium ${!fullWidth ? "lg:hidden xl:block" : ""}`}
    >
      {label}
    </span>
    {active && fullWidth && (
      <div className="ml-auto w-1.5 h-1.5 rounded-full bg-white lg:hidden xl:block" />
    )}
  </a>
);
...
const TransactionItem = ({ title, time, amount, isPositive, icon }) => (
  <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer group">
    ...
    <span
      className={`font-bold ${isPositive ? "text-slate-800" : "text-red-500"}`}
    >
      {amount}
    </span>
  </div>
